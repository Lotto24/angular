...
export const ES_MODULE = getClosureSafeProperty({__esModule: getClosureSafeProperty});

export function isESModule(type: any): type is ESModule {
  return type[ES_MODULE];
}

export function isNgModuleDef<T>(type: any): type is Type<T>&{ɵmod: NgModuleDef<T>} {
